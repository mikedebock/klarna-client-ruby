...
describe Klarna::Methods::ExtendInvoiceDueDate do
  describe '.xmlrpc_name' do
    it 'is extend_invoice_due_date' do
      expect(subject.xmlrpc_name).to eq('extend_invoice_due_date')
    end
    ...
    describe '.xmlrpc_params' do
      context 'given the following input parameters' do
        ...
        it 'returns the invoice due date options in position 3' do
          expect(method_params[3] == extra_info)
        end
      end
    end
  end
